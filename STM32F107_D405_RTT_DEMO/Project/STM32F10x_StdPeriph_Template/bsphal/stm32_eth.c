...
static void RCC_ConfigurationE(void)
{
...
static void NVIC_ConfigurationE(void)
{
 ...
 *******************************************
 ...
 - ETH_MII_RXD3: PD12
 */
static void GPIO_ConfigurationE(void)
{
...
static char eth_phy_thread_stack[512] __attribute__((aligned(4)));
static struct rt_thread eth_phy_thread;

static void eth_phy_thread_entry(void *parameter)
{
    UNUSED(parameter);

    static rt_uint8_t  link_status = 0;
    rt_uint8_t  link_tmp = 0;
    rt_uint16_t res_value;
    static rt_uint16_t last_res_value;

    res_value = ETH_ReadPHYRegister(PHY_ADDRESS, PHY_BSR);
    link_status = (res_value & PHY_Linked_Status) ? 1 : 0;
    while (1) {
        res_value = ETH_ReadPHYRegister(PHY_ADDRESS, PHY_BSR);
		if(last_res_value != res_value) {
			rt_kprintf(">>>>>>>>>> register change: old/new=0x%x/0x%x \n", last_res_value, res_value);
			last_res_value = res_value;
		}
        link_tmp = (res_value & PHY_Linked_Status) ? 1 : 0;
        if (link_tmp != link_status) {
			rt_uint32_t status;
			rt_err_t ret = RT_EOK;
            link_status = link_tmp;
            eth_device_linkchange(&(stm32_eth_device.parent), link_status);
            rt_kprintf(">>>>>>>>>> link change: %d \n", link_status);
			//用event发送通知(mutex容易出问题)
			status = link_status ? MQTT_EVENT_NET_ONLINE : MQTT_EVENT_NET_OFFLINE;
			ret = rt_event_send(mqtt_get_ev(), MQTT_EVENT_NET_CHANGED | status);
...
int eth_phy_thread_startup(void)
{
    rt_err_t result = RT_EOK;

    result = rt_thread_init(&eth_phy_thread, "ephy", eth_phy_thread_entry, RT_NULL,
                            &eth_phy_thread_stack[0], sizeof(eth_phy_thread_stack),
                            RT_LWIP_ETHTHREAD_PRIORITY, 16);
    RT_ASSERT(result == RT_EOK);
    result = rt_thread_startup(&eth_phy_thread);
    RT_ASSERT(result == RT_EOK);
