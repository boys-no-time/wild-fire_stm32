/**
  ...
  *              serial EEPROM (sEE)
  *          available on STM324xG-ATK evaluation board(MB786) RevB from
  *          STMicroelectronics.
  ******************************************************************************
  * @attention
  *
...
/** @addtogroup STM32_ATK
  * @{
  */ 

/** @addtogroup STM324xG_ATK
  * @{
  */   
    
/** @defgroup STM324xG_ATK_LOW_LEVEL 
  * @brief This file provides firmware functions to manage Leds, push-buttons, 
  *        COM ports, SD card on SDIO and serial EEPROM (sEE) available on 
  *        STM324xG-ATK evaluation board from STMicroelectronics.
  * @{
  */ 

/** @defgroup STM324xG_ATK_LOW_LEVEL_Private_TypesDefinitions
  * @{
  */ 
...
/*USART CONFIG*/
USART_TypeDef* COM_USART[COMn] = {DEBUG_USART,ATK_COM3}; 

GPIO_TypeDef* COM_TX_PORT[COMn] = {DEBUG_USART_TX_GPIO_PORT,ATK_COM3_TX_GPIO_PORT};
 
GPIO_TypeDef* COM_RX_PORT[COMn] = {DEBUG_USART_RX_GPIO_PORT,ATK_COM3_RX_GPIO_PORT};

const uint32_t COM_USART_CLK[COMn] = {DEBUG_USART_CLK,ATK_COM3_CLK};

const uint32_t COM_TX_PORT_CLK[COMn] = {DEBUG_USART_TX_GPIO_CLK,ATK_COM3_TX_GPIO_CLK};
 
const uint32_t COM_RX_PORT_CLK[COMn] = {DEBUG_USART_RX_GPIO_CLK,ATK_COM3_RX_GPIO_CLK};

const uint16_t COM_TX_PIN[COMn] = {DEBUG_USART_TX_PIN,ATK_COM3_TX_PIN};

const uint16_t COM_RX_PIN[COMn] = {DEBUG_USART_RX_PIN,ATK_COM3_RX_PIN};
 
const uint16_t COM_TX_PIN_SOURCE[COMn] = {DEBUG_USART_TX_SOURCE,ATK_COM3_TX_SOURCE};

const uint16_t COM_RX_PIN_SOURCE[COMn] = {DEBUG_USART_RX_SOURCE,ATK_COM3_RX_SOURCE};
 
const uint16_t COM_TX_AF[COMn] = {DEBUG_USART_TX_AF,ATK_COM3_TX_AF};
 
const uint16_t COM_RX_AF[COMn] = {DEBUG_USART_RX_AF,ATK_COM3_RX_AF};
...
/**
  ...
  * @retval None
  */
void STM_ATK_KEY_Init()
{
  GPIO_InitTypeDef GPIO_InitStructure;

  /* Enable the BUTTON Clock */
  RCC_AHB1PeriphClockCmd(KEY_PE0_GPIO_CLK , ENABLE);
  
  /* Configure Button pin as input */
	GPIO_InitStructure.GPIO_Pin = KEY_PE0_PIN;
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN;
  GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_NOPULL;
  
  GPIO_Init(KEY_PE0_GPIO_PORT, &GPIO_InitStructure);
}

uint8_t STM_ATK_KEY_Scan(GPIO_TypeDef * GPIOx,uint16_t GPIO_Pin)
{
